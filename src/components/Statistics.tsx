...
export default function Statistics() {
  ...
  const stats = useMemo(() => {
    ...
    return { total, done, todo, todayTasks: todayTasks.length, todayDone, categoryStats }
  ...
  return React.createElement('div', null,
    ...
    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 } },
      ...
      React.createElement('div', { style: cardStyle },
        React.createElement('div', { style: { fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 } }, '完成率'),
        React.createElement('div', { style: { ...statNumStyle, color: '#10b981' } }, rate + '%'),
        React.createElement('div', { style: barTrackStyle },
          React.createElement('div', { style: { height: '100%', width: rate + '%', background: '#10b981', borderRadius: 4, transition: 'width 0.3s' } }),
        ),
      ),
      React.createElement('div', { style: cardStyle },
        React.createElement('div', { style: { fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 } }, '番茄数'),
        React.createElement('div', { style: { ...statNumStyle, color: '#ef4444' } }, state.pomodoroSessions),
        React.createElement('div', { style: { fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 } }, '累计完成专注'),
      ),
    ),
    ...
    React.createElement('div', { style: cardStyle },
      ...
      React.createElement('div', { style: { display: 'flex', gap: 8 } },
        ...(['todo', 'done', 'overdue'] as const).map(s => {
    ...
    stats.total === 0 && state.pomodoroSessions === 0 && React.createElement('div', { className: 'empty-state' },
      React.createElement('div', { className: 'empty-state-text' }, '暂无数据，添加任务或完成番茄钟后查看统计'),
    ),
  )
}
